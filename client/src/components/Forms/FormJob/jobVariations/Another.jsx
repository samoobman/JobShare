...
const Another = () => {
    ...
    useEffect(() => {
        ...
        const timeSpendCount = formJobState.hToWork === 0 || formJobState.hToWork === '' ?
            Math.ceil((workDaysPerY * formJobState.hPerDay) / 24) :
            Math.ceil(((workDaysPerY * formJobState.hToWork * 2) + (workDaysPerY * formJobState.hPerDay)) / 24)
        const sleep = Math.ceil((formJobState.sleepH * 365) / 24)
        const free = Math.ceil(365 - (sleep + timeSpendCount))
        let ySalaryCount
        if (formJobState.exToWork === 0) {
            ySalaryCount = (formJobState.sPerWeek * 52)
        } else {
            ySalaryCount = ((formJobState.sPerWeek * 52) - (formJobState.exToWork * workDaysPerY * 2))
        }
        setJobResult({
            user: userName,
            owner: userId,
    ...
    return (
        <form className={styleSheet.MyForm} onSubmit={e => e.preventDefault()} onClick={e => e.stopPropagation()}>
            <div className={styleSheet.inputWrapper}>
                ...
                <MyInput value={formJobState.daysInWeek} name={"daysInWeek"} onChange={changeHandler} type='number'
                         currentname='Рабочих дней в неделю'/>
                <MyInput value={formJobState.hToWork} name={"hToWork"} onChange={changeHandler} type='number'
                         currentname='Часы на дорогу до работы в один конец'/>
                <MyInput value={formJobState.dVacPerY} name={"dVacPerY"} onChange={changeHandler} type='number'
                         currentname='Дней отпуска в году'/>
                <MyInput value={formJobState.sPerWeek} name={"sPerWeek"} onChange={changeHandler} type='number'
                         currentname='Зарплата в неделю'/>
