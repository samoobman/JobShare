...
const Another = () => {
    ...
    useEffect(() => {
        const workDaysPerY = Math.ceil(((365 - formJobState.dVacPerY) / 7) * 3.7)
        ...
        } else {ySalaryCount = ((formJobState.sPerWeek * 52) - (formJobState.exToWork * workDaysPerY * 2))}
        setJobResult({
            user: userName,
            owner: userId,
    ...
    return (
        <form className={styleSheet.MyForm} onSubmit={e => e.preventDefault()} onClick={e => e.stopPropagation()}>
            <div className={styleSheet.inputWrapper}>
                ...
                <MyInput value={formJobState.hToWork} name={"hToWork"} onChange={changeHandler} type='number' currentname='Часы на дорогу до работы в один конец'/>
                ...
                <MyInput value={formJobState.exToWork} name={"exToWork"} onChange={changeHandler} type='number' currentname='Стоимость дороги до работы'/>
